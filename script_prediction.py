...
# X for independent variables dataset
X = df_order_label[feature_set]

# Customer churn prediction from saved model
customer_churn_model = pickle.load(open('xgboost_v1', 'rb'))
pred_val_customer_churn = customer_churn_model.predict(X)

# create prediction data by copying the input data
df_prediction = df_order_label.copy()
# merge predictions with the dataframe
df_prediction['is_returning_customer']=list(pred_val_customer_churn)
df_prediction = df_prediction[["customer_id", "is_returning_customer"]]
